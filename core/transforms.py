...
def parse_resume(r: Resume) -> dict:
    ...
    skills = tuple(
        re.findall(r"\b(python|java|sql|docker|react|node|c\+\+|git|linux)\b", text)
    )
    exp = (
        int(re.search(r"(\d+)\s+years", text).group(1))
    ...
    edu = (
        "bachelor"
        if "bachelor" in text
        else "master" if "master" in text else "unknown"
    )
    ...
    return {
        ...
        "exp": exp,
        "edu": edu,
        "loc": loc.group(0) if loc else "unknown",
    }
